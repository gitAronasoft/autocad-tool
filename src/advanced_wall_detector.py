...
class AdvancedWallDetector:
    ...
    def detect_all_boundaries(self, vector_paths: list) -> list:
        """
        Extract ALL wall boundaries without classification.
        Returns raw list of boundaries for use with hybrid matching.
        """
        logger.info(f"Extracting all wall boundaries (no classification)...")
        
        # Step 1: Extract ALL vertices from wall paths
        all_vertices, wall_paths = self._extract_all_wall_geometry(vector_paths)
        logger.info(f"  Extracted {len(all_vertices)} unique vertices from {len(wall_paths)} wall paths")
        ...
        # Step 2: Build vertex graph and trace closed loops
        boundaries = self._trace_closed_loops(all_vertices, wall_paths)
        logger.info(f"  Found {len(boundaries)} raw boundary candidates")
    ...
    def _classify_wall_boundaries(self, boundaries: list, page_width: float, page_height: float) -> dict:
        """Classify boundaries using bounding box size and proximity to page edges"""
        if not boundaries:
            return {'exterior_outer': [], 'exterior_inner': [], 'interior_walls': []}
        ...
        for boundary in boundaries:
            ...
            # Calculate bounding box size ratio (how much of page does bbox span)
            width_ratio = bbox_width / page_width if page_width > 0 else 0
            height_ratio = bbox_height / page_height if page_height > 0 else 0
            bbox_size_ratio = (width_ratio + height_ratio) / 2  # Average of width and height ratios
            
            # Calculate distance to page edges (exterior walls should be near edges)
            edge_distance = min(
                min_x,  # distance to left edge
                min_y,  # distance to bottom edge
                page_width - max_x,  # distance to right edge
                page_height - max_y  # distance to top edge
            )
        ...
        # Sort by edge proximity FIRST (exterior walls must be near edges), then bbox size
        # Main walls should be closest to page edges AND have large bounding boxes
        boundary_info.sort(key=lambda b: (-b['edge_distance'], b['bbox_size_ratio'], b['perimeter']), reverse=True)
        
        # Debug: Log top candidates
        logger.info(f"  Top boundary candidates (by edge proximity):")
        for i, b in enumerate(boundary_info[:10]):
            logger.info(f"    #{i+1}: bbox_ratio={b['bbox_size_ratio']:.2%}, edge_dist={b['edge_distance']:.1f}, perimeter={b['perimeter']:.1f}, points={b['num_points']}")
        ...
        if len(boundary_info) >= 2:
            # Outer = boundary with largest bounding box closest to edges
            outer = boundary_info[0]
            result['exterior_outer'] = outer['points']
            logger.info(f"  Selected OUTER: bbox_ratio={outer['bbox_size_ratio']:.2%}, {len(outer['points'])} points")
            ...
            for candidate in boundary_info[1:]:
                ...
                if is_inside and is_significant:
                    ...
                    logger.info(f"  Selected INNER: bbox_ratio={candidate['bbox_size_ratio']:.2%}, {len(candidate['points'])} points")
                    # Rest are interior
